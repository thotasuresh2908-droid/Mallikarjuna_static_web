    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 pt-24 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          {/* Hero Card */}
          <section id="home" className="lg:col-span-4 lg:row-span-1 bg-white rounded-[2rem] p-8 md:p-12 border border-slate-200 shadow-sm flex flex-col justify-center relative overflow-hidden group min-h-[400px]">
            ...
            >
              ...
              <div className="flex gap-4">
                ...
                <a href="#gallery" className="bg-white text-slate-900 border border-slate-200 px-6 py-3 rounded-2xl font-bold text-sm hover:bg-slate-50 transition-all flex items-center gap-2">
                  View Shop Photos
                </a>
              </div>
            </motion.div>
          ...
          {/* Social / Media Feed (Wide Column) */}
          <section id="media" className="lg:col-span-1 lg:row-span-1 bg-slate-900 rounded-[2rem] p-8 text-white flex flex-col group min-h-[500px]">
            ...
            <div className="space-y-6 flex-grow overflow-y-auto custom-scrollbar pr-1">
              {/* Instagram Profile Embed / Live Pulse */}
              <div className="bg-white/5 rounded-3xl p-1 border border-white/10 hover:bg-white/10 transition-colors overflow-hidden group/pulse">
                <div className="aspect-[9/16] bg-slate-900 rounded-2xl overflow-hidden relative shadow-2xl">
                   <iframe 
                      src={`${ASSETS.INSTAGRAM_PROFILE}embed`}
                      className="w-full h-full border-0 absolute inset-0"
                      title="Instagram Live Feed"
                    ></iframe>
                </div>
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Instagram className="w-4 h-4 text-pink-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Instagram Feed</span>
                  </div>
                  <p className="text-sm font-bold leading-snug">Our latest reels and posts sync here automatically!</p>
                </div>
              </div>

              {/* Shop Tour Highlights (Mini) */}
              <div className="bg-indigo-600/20 rounded-2xl p-4 border border-indigo-500/30">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-300 mb-2">Shop Pulse</h4>
                <p className="text-xs font-medium text-slate-300">
                  New arrivals updated daily. Follow us to never miss an update!
                </p>
              </div>
            </div>

            <div className="mt-8 pt-6 border-t border-white/5 flex justify-around">
               <a href="https://youtube.com/@shiva_ganesh_ready_mades" target="_blank" rel="noopener noreferrer">
                 <Youtube className="w-5 h-5 text-slate-500 hover:text-white transition-colors cursor-pointer" />
               </a>
               <a href={ASSETS.INSTAGRAM_PROFILE} target="_blank" rel="noopener noreferrer">
        ...
        {/* Full Store Gallery Section */}
        <section id="gallery" className="lg:col-span-4 mt-12 bg-white rounded-[2rem] p-8 md:p-12 border border-slate-200 shadow-sm relative overflow-hidden">
          <div className="absolute -left-20 -bottom-20 w-64 h-64 bg-indigo-50/30 rounded-full blur-3xl -z-10"></div>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 px-2 relative z-10">
            <div>
              <h2 className="text-3xl font-black text-slate-800 tracking-tight mb-2">Shop Gallery</h2>
              <p className="text-slate-500 font-bold text-xs uppercase tracking-widest leading-relaxed">
                Take a virtual tour of <span className="text-indigo-600">Mallikarjuna Plastics</span>. Showing all shop photos from Google & Store front.
              </p>
            </div>
            <div className="hidden md:flex gap-2">
              <div className="px-4 py-2 bg-slate-100 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-400">
                {SHOP_IMAGES.length} Photos Syncing
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {SHOP_IMAGES.map((img, i) => (
              <motion.div 
                ...
                className="relative rounded-[1.5rem] md:rounded-[2rem] overflow-hidden group border border-slate-200 shadow-sm aspect-square md:aspect-[4/5]"
              >
                <SmartImage 
                  src={img} 
                  alt={`Shop View ${i+1}`} 
        ...
        {/* FEEDBACK / MEDIA HIGHLIGHT SECTION (Restored) */}
          <section className="lg:col-span-4 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mt-6">
            <div className="md:col-span-3 bg-white rounded-[2rem] p-6 border border-slate-200 shadow-sm flex flex-col group min-h-[320px]">
              <div className="flex items-center justify-between mb-4 px-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Live Shop Tour</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-rose-500 rounded-full animate-ping"></div>
                  <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest bg-rose-50 px-4 py-1.5 rounded-full border border-rose-100">Live Preview</span>
                </div>
              </div>
              <div className="flex-grow aspect-video bg-slate-900 rounded-[1.5rem] overflow-hidden relative shadow-2xl">
                    <iframe 
                      className="absolute inset-0 w-full h-full border-0"
                      src={ASSETS.VIDEOS.TOUR} 
